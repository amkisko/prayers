...
Run only on a target the user is authorized to investigate (they operate it, they have written permission, or it is in a named bounty or disclosure scope). Stop if authorization is missing. This package is reconnaissance, not an autonomous pentest runner. Redact personal data by default. Do not spoof a User-Agent to bypass a block or quote live secret-file bodies.
